...
{
    // Start is called before the first frame update    #region variables
    ...
    {
        return new Vector3(
            GetFloat(transform.localPosition.x),
            GetFloat(transform.localPosition.y+5f),
            //deapth modifier applied here
            GetFloat(transform.localPosition.z) * traumaDepthMag
            );
    }
